...
local Mist_murderer_progress = { 	{0 ,"mist_murderer_0", 1},
...
local Ulthuan_regions = {
			...
			"wh2_main_eataine_tower_of_lysean",
			"wh2_main_ellyrion_tor_elyr",
			"wh2_main_eagle_gate",
			"wh2_main_ellyrion_whitefire_tor",
			...
			"wh2_main_saphery_tower_of_hoeth",
			"wh2_main_saphery_port_elistor",
			"wh2_main_eataine_angerrial",
			"wh2_main_eataine_shrine_of_asuryan",
			--vortex
			...
			"wh2_main_vor_straits_of_lothern_tower_of_lysean",
			"wh2_main_vor_ellyrion_the_arc_span",
			"wh2_main_vor_eagle_gate",
			"wh2_main_vor_ellyrion_tor_elyr",
			...
			"wh2_main_vor_saphery_shadow_peak",
			"wh2_main_vor_saphery_tower_of_hoeth",
			"wh2_main_vor_saphery_port_elistor",
			"wh2_main_vor_eataine_angerrial",
			"wh2_main_vor_eataine_shrine_of_asuryan"
	};
...
--the goal of recipe collection, used in vortex final battle trigger, notice this has to keep synced with grom cauldron script
local Goal_number_recipe = 12;
...
local BlacktoofMissionsPayload = {{}, {"faction_pooled_resource_transaction{resource grn_salvage;factor wh2_dlc12_resource_factor_loot;amount 100;}", "money 1200"}, {"faction_pooled_resource_transaction{resource grn_salvage;factor wh2_dlc12_resource_factor_loot;amount 150;}", "money 1500"}, {"effect_bundle{bundle_key wh2_dlc15_grom_unlock_special_recipe;turns 0;}", "money 1500"}, {"effect_bundle{bundle_key wh2_dlc15_grn_narration_groms_ready;turns 0;}"}, {}
};
...
function add_grom_story_listeners()
	...
	--resend info to UI if new/old vortex game
	if cm:model():campaign_name("wh2_main_great_vortex") then
		local component = find_uicomponent(core:get_ui_root(), "grom_goals");
		if component then
			component:InterfaceFunction("SetEltharionChallengeRecipeCount", Goal_number_recipe);
			--out("setup vortex obj for grom?@@@@!");
		end
	end
	...
	if not cm:get_faction(Eltharion_faction):is_null_interface() and not cm:get_faction(Eltharion_faction):is_human() then
		...
		if cm:is_new_game() then
			...
			if cm:model():campaign_name("wh2_main_great_vortex") then
				cm:trigger_incident(Grom_faction, "wh2_dlc15_grom_yvresse_blockade", true);
				cm:cai_disable_targeting_against_settlement("settlement:wh2_main_vor_northern_yvresse_tor_yvresse");
				--give yvresse some defense resource
				cm:faction_add_pooled_resource(Eltharion_faction, "yvresse_defence", "wh2_dlc15_resource_factor_yvresse_defence_upgrade", 80);
				ShouldLockYvresse = true;
		...
		cm:add_faction_turn_start_listener_by_name(
			...
			function(context)
				...
				check_and_process_periodic_mist_murderer();
		...
		core:add_listener(
			...
			function(context)
				...
				if mission_key == BlacktoofMissions[#BlacktoofMissions] then
					...
					ShouldLockYvresse = false;
					local region = cm:get_region(Mist_murderer_location["wh2_main_great_vortex"]);
					if not region:is_null_interface() and (region:owning_faction():is_null_interface() or (not region:owning_faction():is_null_interface() and not region:owning_faction():is_human())) then
						cm:transfer_region_to_faction(Mist_murderer_location["wh2_main_great_vortex"], Grom_faction);
					end
				end	
				...
				if mission_key == "wh2_dlc15_grn_grom_black_toof_4" then
					cm:complete_scripted_mission_objective("wh_main_long_victory", "complete_blacktoof_revenge", true);
				end	
			end,
		true
		);
	...
	core:add_listener(
			...
			function(context)
				...
				if context:pending_battle():seige_battle() and in_yvresse and ShouldLockYvresse then
					return true;
				else
					return false;
				end
	...
	core:add_listener(
		...
		"MissionSucceeded",
		function(context)
			return cm:model():campaign_name("wh2_main_great_vortex") and context:mission():mission_record_key() == BlacktoofMissions[#BlacktoofMissions];
		end,
		function()
			core:svr_save_registry_bool("warden_grom_win", true);
			cm:register_instant_movie("warhammer2/warden/warden_grom_win");
		end,
		false
	);
	...
	core:add_listener(
		...
		"MissionSucceeded",
		function(context)
			return cm:model():campaign_name("wh2_main_great_vortex") and context:mission():mission_record_key() == BlacktoofMissions[#BlacktoofMissions - 1];
		end,
		function()
			core:svr_save_registry_bool("warden_grom_call_to_arms", true);
			cm:register_instant_movie("warhammer2/warden/warden_grom_call_to_arms");
		end,
		false
	);
...
function setup_black_toof_prophecies_listener()
	core:add_listener(
		...
		function(context)
			cm:complete_scripted_mission_objective("wh2_dlc15_grom_blacktoof_prophecy_0", "prophecies_0", true);
		end,
	true
	);
end
...
function check_and_process_periodic_mist_murderer()
	--setup the yvresse take back force, this is more frequent than normal ones but only in Vortex 
	if cm:model():campaign_name("wh2_main_great_vortex") and ShouldLockYvresse then
		local settlement = cm:get_region(Mist_murderer_location["wh2_main_great_vortex"]):settlement();
		if (not settlement:is_null_interface() and not settlement:faction():is_null_interface() and settlement:faction():name() == Grom_faction) or (settlement:is_null_interface()) then
			if Mist_murderer_count_down <= 0 then
				spawn_a_mist_murderer(nil, Mist_murderer_location["wh2_main_great_vortex"], false);
				Mist_murderer_count_down = Mist_murderer_cooldown;
			else
				Mist_murderer_count_down = Mist_murderer_count_down - 2;
			end
		end
	end
	
	--setup the periodic mist murder only if eltharion is alive
	-- if cm:get_faction(Eltharion_faction):is_null_interface() then
		-- return;
	-- end
	--we are not using this periodic spawning, so commented out
	-- if Mist_murderer_count_down <= 0 then
		-- spawn_a_mist_murderer(nil, nil, false);
		-- Mist_murderer_count_down = Mist_murderer_cooldown;
	-- else
		-- Mist_murderer_count_down = Mist_murderer_count_down - 1;
	-- end
end
...
function setup_black_toof_scripted_mission(index)
	...
	if BlacktoofMissions_ME_override[index] ~= "" and cm:get_campaign_name() == "main_warhammer" then
...
function setup_black_toof_mission_listener(index)
	if index == 2 then
		...
		core:add_listener(
			...
			function(context)
				cm:complete_scripted_mission_objective(BlacktoofMissions[index], "grom_mission_"..tostring(index), true);
			end,
		true
		);
	elseif index == 3 then
...
function spawn_a_mist_murderer(target, target_region_key, goes_to_the_list)
	...
	local loc = {-1, -1};
	if target == nil and cm:model():campaign_name("wh2_main_great_vortex") then
		loc[1], loc[2] = cm:find_valid_spawn_location_for_character_from_position(Mist_murderer_faction, 652, 517, true, radius_yvresse);
	elseif target == nil and cm:model():campaign_name("main_warhammer") then	
