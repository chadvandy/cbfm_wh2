...
function add_beast_tech_lock_listeners()
	...
	if cm:is_new_game() == true then
		...
		for j = 1, #beastmen_factions do	
			local faction = cm:get_faction(beastmen_factions[j])
			--This will check the factions that are available and lock the techs for them while skipping the non available ones without errors
			if faction then
				local faction_key = faction:name();
				for i = 1, #lock_tech_keys do
					cm:lock_technology(faction_key, lock_tech_keys[i]);
				end	
				...
				if faction:is_human() == true then
					if beastmen_players[faction_key] == nil then
						...
						beastmen_players[faction_key].battle_win = 0;
						beastmen_players[faction_key].rituals_cast = 0;
						beastmen_players[faction_key].agent_actions = 0;
						beastmen_players[faction_key].percentage_wins = 0;
						beastmen_players[faction_key].climates_counter = 0;
						beastmen_players[faction_key].culture_counter = 0;
						beastmen_players[faction_key].same_turn_battle_wins = 0;
						beastmen_players[faction_key].unfavoured_counter = 0;
						beastmen_players[faction_key].summon_counter = 0;
						beastmen_players[faction_key].faction_leader_counter = 0;
						beastmen_players[faction_key].climate_done = {};
						beastmen_players[faction_key].culture_done = {};
						beastmen_players[faction_key].techs_unlocked = {};
					end
				end
...
function TechLockUnlockListener(key, beastmen_player)
	...
	core:add_listener(
		...
		function (context)
			...
			if cm:char_is_general_with_army(character) and character:won_battle() and context:pending_battle():seige_battle() == true and context:pending_battle():battle_type() == "settlement_standard" then
		...
		function(context)
			...
			if cm:pending_battle_cache_num_attackers() >= 1 then
				for i = 1, cm:pending_battle_cache_num_attackers() do
					...
					for j = 1, #beastmen_factions do
						local faction = cm:get_faction(beastmen_factions[j]) 
						if current_faction_name == beastmen_factions[j] and faction:is_human() then
	...
	core:add_listener(
		...
		function (context)
			...
			if cm:char_is_general_with_army(character) and beastmen_player.summon_counter >= summon_threshold and not faction:has_technology("tech_dlc17_bst_fifteen_spells") then
	...
	core:add_listener(
		...
		function(context)
			return key == context:character():faction():name() and (context:occupation_decision() == "861999871" or context:occupation_decision() == "339648469" or context:occupation_decision() == "2083797902");
		end,
		function(context)
			local character = context:character();
