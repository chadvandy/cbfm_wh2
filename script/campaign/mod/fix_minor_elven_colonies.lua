function fix_minor_elven_colonies()
    ...
    --if not cm:get_saved_value("fix_minor_elvencolonies") then
        for , region_name in pairs(minor_elven_colonies) do
            ...
            if ca_region and not ca_region:is_abandoned() and ca_region:owning_faction():subculture() == "wh_main_sc_brt_bretonnia" then
                ...
                local levelstring = string.match(building, '(%d)')
                cm:region_slot_instantly_upgrade_building(ca_region:settlement():primary_slot(), "wh2_main_special_settlement_colony_minorbrt"..level_string)
            end
        end
