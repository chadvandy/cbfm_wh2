local dwarf_faction_key = "wh_main_dwf_dwarfs";
...
function add_grombrindal_listeners()
	local is_grombrindal = false;
	
	if cm:is_new_game() then
		is_grombrindal = cm:general_with_forename_exists_in_faction_with_force(dwarf_faction_key, "names_name_2147358917");
		cm:set_saved_value("grombrindal_starting_lord", is_grombrindal);
	else
		is_grombrindal = cm:get_saved_value("grombrindal_starting_lord");
	end;
	
	if is_grombrindal then
	...
	if god_choice_last_god ~= "none" and cm:model():turn_number() > 2 then
		...
		if faction and character then
			...
			if character:get_forename() == "names_name_2147358917" and character:has_military_force() then
				cm:apply_effect_bundle_to_characters_force(effect, cqi, 0, true);
			end;
		end;
	end;
end;
